...
class Decoder : public DecoderInterface {
    ...
    SchmidlCox<float, cmplx, search_position, symbol_length / 2, guard_length>
            correlator;
    ...
    cmplx temp[extended_length], freq[symbol_length], prev[pay_car_cnt],
            cons[pay_car_cnt];
    ...
    const cmplx *corSeq() {
        ...
        for (int i = 0; i < cor_seq_len; ++i)
            freq[(i + cor_seq_off / 2 + symbol_length / 2) %
                    (symbol_length / 2)] = nrz(seq());
        return freq;
    }

    cmplx analytic(float real) { return hilbert(block_dc(real)); }
    ...
    cmplx convert(const int16_t *samples, int channel, int i) {
        ...
        case 3:
            return analytic(
                    ((int)samples[2 * i] + (int)samples[2 * i + 1]) / 65536.f);
        case 4:
    ...
    void update_spectrogram(uint32_t *pixels) {
        std::memmove(pixels + spectrogram_width, pixels,
                sizeof(uint32_t) * spectrogram_width *
                        (spectrogram_height - 1));
    ...
    void demap() {
        float pre = precision();
        for (int i = 0; i < pay_car_cnt; ++i)
            mod_soft(code + mod_bits * (symbol_number * pay_car_cnt + i),
                    cons[i], pre);
    }
    ...
    int preamble() {
        ...
        for (int i = 0; i < pre_seq_len; ++i)
            PhaseShiftKeying<2, cmplx, int8_t>::soft(soft + i,
                    demod_or_erase(freq[bin(i + pre_seq_off)],
                            freq[bin(i - 1 + pre_seq_off)]),
                    32);
...
public:
    ...
    bool feed(const int16_t *audio_buffer, int sample_count,
            int channel_select) final {
    ...
    int process() final {
        ...
        if (symbol_number < symbol_count) {
            ...
            if (symbol_number >= 0) {
                for (int i = 0; i < pay_car_cnt; ++i)
                    cons[i] =
                            demod_or_erase(freq[bin(i + pay_car_off)], prev[i]);
                compensate();
                demap();
            }
            if (++symbol_number == symbol_count)
    ...
    void spectrum(uint32_t *spectrum_pixels, uint32_t *spectrogram_pixels,
            int spectrum_tint) final {
        for (int j = 0; j < 2; ++j) {
            for (int i = 0; i < stft_length; ++i)
                temp[i] = 0;
            for (int i = 0; i < window_length; ++i)
                temp[i % stft_length] +=
                        window[i] * buf[buffer_length - window_length +
                                            stft_length * (j - 1) + i];
            stft(freq, temp);
            for (int i = 0; i < spectrum_width; ++i)
                power[i] = std::clamp<float>(
                        (DSP::decibel(norm(freq[i])) - dB_min) /
                                (dB_max - dB_min),
                        0, 1);
