...
class Encoder : public EncoderInterface {
    ...
    void schmidl_cox() {
        ...
        for (int i = 0; i < cor_seq_len; ++i)
            freq[bin(2 * i + cor_seq_off)] *=
                    freq[bin(2 * (i - 1) + cor_seq_off)];
        transform(false);
    }
    ...
    void fancy_symbol() {
        ...
        for (int j = 0; j < 9; ++j)
            for (int i = 0; i < 8; ++i)
                active_carriers +=
                        (base37_bitmap[call[j] + 37 * fancy_line] >> i) & 1;
        ...
        for (int j = 0; j < 9; ++j)
            for (int i = 0; i < 8; ++i)
                if (base37_bitmap[call[j] + 37 * fancy_line] & (1 << (7 - i)))
                    freq[bin((8 * j + i) * 3 + fancy_off)] =
                            factor * nrz(noise_seq());
        transform(false);
    }
    ...
    void noise_symbol() {
        ...
        for (int i = 0; i < symbol_length; ++i)
            freq[i] = 0;
        for (int i = 0; i < pay_car_cnt; ++i)
            freq[bin(i + pay_car_off)] =
                    factor * cmplx(nrz(noise_seq()), nrz(noise_seq()));
        transform(false);
    }
    ...
    void payload_symbol() {
        for (int i = 0; i < symbol_length; ++i)
            freq[i] = 0;
        for (int i = 0; i < pay_car_cnt; ++i)
            freq[bin(i + pay_car_off)] = prev[i] *= mod_map(
                    code + mod_bits * (pay_car_cnt * symbol_number + i));
        transform();
    }
    ...
    void next_sample(int16_t *samples, cmplx signal, int channel, int i) {
        switch (channel) {
        case 1:
            samples[2 * i] = std::clamp<float>(
                    std::nearbyint(32767 * signal.real()), -32768, 32767);
            samples[2 * i + 1] = 0;
            break;
        case 2:
            samples[2 * i] = 0;
            samples[2 * i + 1] = std::clamp<float>(
                    std::nearbyint(32767 * signal.real()), -32768, 32767);
            break;
        case 4:
            samples[2 * i] = std::clamp<float>(
                    std::nearbyint(32767 * signal.real()), -32768, 32767);
            samples[2 * i + 1] = std::clamp<float>(
                    std::nearbyint(32767 * signal.imag()), -32768, 32767);
            break;
        default:
            samples[i] = std::clamp<float>(
                    std::nearbyint(32767 * signal.real()), -32768, 32767);
        }
    }
...
public:
    ...
    bool produce(int16_t *audio_buffer, int channel_select) final {
        ...
        for (int i = 0; i < guard_length; ++i) {
            ...
            cmplx sum = DSP::lerp(
                    guard[i], temp[i + symbol_length - guard_length], y);
        ...
        for (int i = 0; i < symbol_length; ++i)
            next_sample(
                    audio_buffer, temp[i], channel_select, i + guard_length);
        return true;
    }
    ...
    void configure(const uint8_t *payload, const int8_t *call_sign,
            int carrier_frequency, int noise_symbols, bool fancy_header) final {
        int len = 0;
